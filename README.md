...
The tool output is real, captured by [`scripts/build_media.py`](scripts/build_media.py) running
`server.py` over MCP; the final assistant reply is hand-written for illustration.*
...
- Ranking is simple keyword scoring (topic 10, tag 8, path 5, summary 3 per term), not semantic search.
  Space-separated terms are OR-ed and matched as substrings, so short or very common words add noise.
- Tested on Windows; the code uses plain `os.path` and should run elsewhere, but that is unverified.
